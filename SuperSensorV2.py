...
def getCPU():
    ...
    if osInfo != "Windows":
        ...
        loadOut = [
            {'channel': 'CPU 1M Load %', 'value': cpuAvg[0], 'float': '1', 'customunit': '%', 'limitmaxwarning': '90',
             'limitmode': '1'},
            {'channel': 'CPU 5M Load %', 'value': cpuAvg[1], 'float': '1', 'customunit': '%', 'limitmaxwarning': '75',
             'limitmode': '1'},
            {'channel': 'CPU 15M Load %', 'value': cpuAvg[2], 'float': '1', 'customunit': '%', 'limitmaxwarning': '50',
             'limitmaxerror': '75', 'limitmode': '1'}]
...
def getMEM():
    ...
    if swapTotal > 0:
        ...
        swapOut = [{'channel': 'Total Swap', 'value': swapTotal, 'float': '1', 'customunit': 'GB'},
                   ...
                   {'channel': 'Swap Utilization %', 'value': round(((swapUsed / swapTotal) * 100), 2), 'float': '1',
                    'customunit': '%', 'limitmaxwarning': '20', 'limitmaxerror': '50', 'limitmode': '1'}]
...
def getNET():
    ...
    netOut = [{'channel': 'Network Sent', 'value': round((net.bytes_sent / kB), 2), 'float': '1', 'customunit': 'GB'},
              {'channel': 'Network Received', 'value': round((net.bytes_recv / kB), 2), 'float': '1',
               'customunit': 'GB'},
        ...
        if disks[counter].opts != "cdrom":
            ...
            diskIn = [{'channel': 'Disk Total ' + volume + ':', 'value': diskTotal, 'float': '1', 'customunit': 'GB'},
                      ...
                      {'channel': 'Disk Utilization ' + volume + ':', 'value': round(((diskFree / diskTotal) * 100), 2),
                       'float': '1', 'customunit': '%', 'limitmaxwarning': '85', 'limitmaxerror': '95',
                       'limitmode': '1'}]
